...
logging.basicConfig(format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
                    level=logging.INFO)
text = "Share some location to get a weather picture or send /Konigsberg command to see the picture for Kaliningrad.\nHow to read forecast: /legend"
...
def konigsberg(update, context):
    ...
    if randrange(10) == 0:
        context.bot.send_message(chat_id=update.message.chat_id, text="Try to share any location to get forecast for that place! 📍")
